...
// ═══════════════════════════════════════════════════════════════
// MAIN: Generate image via direct API call
// ═══════════════════════════════════════════════════════════════
async function automateGeneration(page, prompt, filePayloads, config, baseDir) {
  ...
  console.log(`[Gen] mode=${mode} model=${imageModel} ratio=${aspectRatio} outputs=${outputCount}`);

  let tempFilePaths = [];

  try {
    // ── Step 1: Navigate to project page & switch to Image mode ──
    console.log('[Gen] Step 1: Navigating to project page...');
    // await page.goto(PROJECT_URL);
    // await page.waitForTimeout(5000);

    // Switch to Image mode via settings popup (in case video mode was active)
    ...
    if (filePayloads && filePayloads.length > 0) {
      ...
      console.log('[Gen] Step 2: No images to upload, skipping.');
    }

    // ── Step 3: Resolve image UUIDs ──
    let imageInputUUIDs = [];
    if (config.imageSelection && config.imageSelection.length > 0) {
      ...
      for (const sel of config.imageSelection) {
        if (sel.startsWith('name:')) {
          ...
          if (uuid) {
            ...
            console.log(`[Gen] ⚠️ Skipping unresolved reference: "${sel}"`);
          }
      ...
      // If images were uploaded but no imageSelection, resolve uploaded file names
      console.log(`[Gen] Step 3: Resolving uploaded image UUIDs...`);
      for (const fp of filePayloads) {
        const uuid = await findImageUUID(page, fp.name);
        if (uuid) imageInputUUIDs.push(uuid);
      }
    } else {
      console.log('[Gen] Step 3: No image references to resolve.');
    }

    // ── Step 4: Get tokens (reload page for fresh reCAPTCHA context) ──
    console.log('[Gen] Step 4: Reloading page for fresh reCAPTCHA context...');
    // await page.goto(PROJECT_URL);
    // await page.waitForTimeout(5000);
    ...
    // ── Step 5: Call batchGenerateImages API ──
    ...
    const requestBody = {
      ...
      requests: [{
        ...
        seed,
        imageInputs: imageInputUUIDs.map(name => ({
    ...
    const response = await context.request.fetch(apiUrl, {
      ...
      timeout: 120000  // 2 minutes — image gen can take 30-60s
    });
    ...
    if (status !== 200) {
      throw new Error(`[Gen] API returned HTTP ${status}: ${body.substring(0, 500)}`);
    }

    const result = JSON.parse(body);
    console.log(`[Gen] ✅ API returned successfully!`);

    // ── Step 6: Extract image URL and fetch as base64 ──
    const media = result.media || [];
    if (media.length === 0) {
      throw new Error('[Gen] API returned no media in response');
    }

    console.log(`[Gen] Step 6: Fetching ${media.length} generated image(s) as base64...`);
    ...
    if (!fifeUrl) {
      throw new Error('[Gen] No fifeUrl in API response');
    }

    // Fetch the image as base64 using context.request (no CORS)
    ...
    console.log(`[Gen]   ✅ Main image fetched (base64 length: ${resultBase64.length}).`);

    // Fetch all results if outputCount > 1
    let allResults = [];
    if (outputCount > 1 && media.length > 1) {
      console.log(`[Gen]   Fetching all ${media.length} outputs...`);
